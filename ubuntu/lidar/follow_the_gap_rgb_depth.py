...
class FollowTheGapRgbDepth(Node):
    def __init__(self):
        ...
        self.declare_parameter("gap_min_bins", 10)
        self.declare_parameter("center_weight", 0.08)
        self.declare_parameter("rotate_yaw_threshold_deg", 8.0)
        self.declare_parameter("align_yaw_speed", 0.35)
        ...
        self.rotate_yaw_threshold = np.radians(float(g("rotate_yaw_threshold_deg")))
        self.align_yaw_speed = float(g("align_yaw_speed"))
        self.kp = float(g("kp"))
    ...
    def image_callback(self, msg: Image):
        ...
        if front_clear < self.front_stop:
            ...
            if abs(target_angle) > self.rotate_yaw_threshold:
                yaw = float(np.sign(target_angle) * min(self.align_yaw_speed, self.max_w))
                speed, state = 0.0, "RGB_ALIGN"
            else:
    ...
    def find_best_gap(self, scan: np.ndarray):
        ...
        best_bin = max(candidates, key=lambda idx: float(scan[idx]) - self.center_weight * abs(idx - center))
...
def _self_test():
    ...
    scan, _, _ = estimate_visual_scan(img, 20, 0.25, 1.0, 0.15, 0.5, 30.0, 28.0, 0.08, 2, 0.35, 4.0)
    assert float(np.min(scan[8:12])) < 1.0
    assert float(np.mean(scan[3:5])) > 3.0
    assert abs(float(np.max(scan[:2])) - 0.35) < 1e-5

...
class FollowTheGap(Node):
    def __init__(self):
        ...
        self.declare_parameter(
            "bubble_radius", 0.35
        )  # safety bubble around nearest pt (m)
        self.declare_parameter("gap_threshold", 0.60)  # range above this = "free" (m)
    ...
    def cb(self, scan: LaserScan):
        ...
        side_error = left_clear - right_clear
        yaw = float(
            np.clip(
                self.steer_sign * self.side_gain * side_error,
                -self.max_forward_yaw,
                self.max_forward_yaw,
            )
        )
        state = "FORWARD"
        if front_clear < self.front_stop:
            ...
            speed = 0.0
            state = "FRONT_BLOCKED"
        elif min(left_clear, right_clear) < self.side_stop:
            yaw = self._escape_yaw(left_clear, right_clear)
            speed = 0.0
            state = "SIDE_BLOCKED"
        elif abs(yaw) >= self.rotate_yaw_threshold:
            yaw = float(np.sign(yaw) * min(self.escape_yaw, self.w_max))
            speed = 0.0
            state = "REALIGN"
        else:
            ...
            speed = self.v_min + (self.v_max - self.v_min) * front_factor * turn_factor

...
class UdpTelemetryNode(Node):
    def __init__(self):
        ...
        self.last_qr_text = None      # last QR sent; avoid re-sending same code
        self._qr_frame_counter = 0
        self._qr_check_every = 10     # run detection every N frames (~3x/sec at 30fps)
    ...
    def image_callback(self, msg: Image):
        ...
        try:
            ...
            # ========= QR Detection (rate-limited) =========
            self._qr_frame_counter += 1
            if self._qr_frame_counter % self._qr_check_every == 0:
                qr_text, _, _ = self.qr_detector.detectAndDecode(cv_img)
                if qr_text:
                    if qr_text != self.last_qr_text:
                        self.last_qr_text = qr_text
                        header = (
                            f"QR {self.ros_domain_id} {self.robot_name} "
                            f"{msg.header.stamp.sec} {msg.header.stamp.nanosec}"
                        )
                        self.sock.sendto(
                            f"{header} {qr_text}".encode("utf-8"),
                            self.authorized_addr,
                        )
                        self.get_logger().info(f"QR enviado: '{qr_text}'")
                else:
                    # Reset so the same code can be sent again after disappearing

#!/usr/bin/env python3
"""
Competition navigation from the OAK-D RGB preview only.
...
This variant keeps the working RGB follow-the-gap behavior from v0 and adds the
minimum competition behaviors: QR logging and traffic-signal actions read from
the laptop YOLO latest_signal.json file.
"""
...
class CompetitionRgbGapQrSignalNav(Node):
    def __init__(self):
        ...
        self.declare_parameter("gap_min_bins", 7)
        ...
        self.get_logger().warn("Using RGB pseudo-depth only; run slowly and supervise the robot.")
    ...
    def image_callback(self, msg: Image):
        ...
        speed, yaw, state = self.decide_motion(scan, best_bin, front_clear, signal)
    ...
    def decide_motion(self, scan, best_bin, front_clear, signal):
        now = time.monotonic()
        if front_clear < self.front_stop:
            self.active_maneuver = None
            return 0.0, self.escape_yaw(scan), "RGB_FRONT_BLOCKED"
        ...
        if best_bin is None:
            return 0.0, self.escape_yaw(scan), "RGB_BLOCKED"
    ...
    def read_latest_signal(self):
        ...
        actionable = (
            ...
            and direction in ("left", "right", "stop")
    ...
    def escape_yaw(self, scan: np.ndarray):
        ...
        return float(np.clip(self.escape_w if left > right else -self.escape_w, -self.max_w, self.max_w))

...
#  - Lee output/handcraft/latest_signal.json
...
LATEST_SIGNAL_PATH = REPO_ROOT / "output" / "handcraft" / "latest_signal.json"
...
def read_latest_signal():
    ...
    if signal_type not in ("arrow", "qr", "none"):
        return None
    if direction not in ("left", "right", "none"):
        return None
    if signal_type != "arrow" or direction == "none":
        return payload
...
def main():
    ...
    print("=== UDP Teleop + Handcraft Auto Turn (Windows) ===")
        ...
        while True:
            ...
            if not auto_direction:
                ...
                if can_start:
                    ...
                    manual_v, manual_w = 0.0, 0.0
                    print(
                        f"\n[AUTO] Flecha {auto_direction} "
                        f"score={float(signal.get('confidence', 0.0)):.2f}. "

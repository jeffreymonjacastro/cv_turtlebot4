...
# ========= Configuración =========
ROBOT_IP = "172.31.245.200"  # IP del TurtleBot4
        ...
        try:
            ...
            if len(parts) >= 3 and parts[0] == "ACK":
                ...
                if domain_id != DESIRED_DOMAIN_ID:
                    print(
                        f"[HANDSHAKE] ROS_DOMAIN_ID no coincide "
                ...
                if robot_name != EXPECTED_ROBOT_NAME:
                    print(
                        f"[HANDSHAKE] robot_name no coincide "
...
def handle_scan(parts):
    ...
    try:
        ...
        if len(ranges_str) != n:
            print(
                f"[SCAN] n={n} pero llegaron {len(ranges_str)} rangos. Usando min(len, n)."
            )
        ...
        print(
            f"[SCAN] robot={robot_name} domain={domain_id} "
